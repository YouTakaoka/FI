...
\newtcbtheorem[number within=section]{meth}{解法}{enhanced, %TikZの内部処理を導入する．ある程度複雑なものには必須．
    attach boxed title to top left={xshift=5mm,yshift=-3mm}, 
    boxed title style={colframe = green!35!black, colback = white},
    coltitle = black,
    colback = white,
...
\subsection{整数の概念}
    ...
    とどこまでも果てしなく続いていく記号の羅列である\footnote{
        自然数に$0$を含めるかどうかには，流儀ごとに違いがある．
        筆者としては，そんなことは本質的な問題ではないという意見であるし，
        文献ごとにちゃんと断って使用すればどちらでもよいと思う．
        このテキストでは，ひとまず$0$は\emphasize{含めない}流儀でいく．
        ここでの自然数を\ind{正整数}{せいせいすう}ともいう．
    }．
    たとえば"$2$"という数であれば，
    \begin{itemize}
        \item $2$個のりんご
...
\subsection{進法とは何か}
    ...
    ところで，上で出てきた"$10$"は明らかに，"$10$"進法の$10$である．
    もしこれが$N$進法になれば，話は上で出てきた"$10$"を"$N$"で置き換えるだけで
    済むことは明らかだろう．
    もう一度要点をまとめておくと，$N$進法とは正整数\footnote{
        \ind{正整数}{せいせいすう}とは$0$より大なる整数，
        すなわちこのテキストの定義だと自然数と同義である．
        ただし，既に述べたように「自然数」という用語は文献ごとに意味の揺れが激しいのと，
        数学的に指すものが同じでも用語が違うと少しニュアンスが異なる場合があり，
        この場合「整数全体を考えている文脈で，一旦$0$より大きいやつらを抜き出しますよ」
        という細かな意味合いを表現したかったのでこちらを選んだ．
    }$x$を
    \begin{equation}
        (x_n \cdots x_1 x_0)_N   \label{eq:integer-radix-N-expression}
    \end{equation}
    のように書き表わす仕方である．ただし，このとき各桁$x_i(i=0,1,...,n)$は
    \begin{equation*}
        x_iは整数,\ 0 \leq x_i < N
    \end{equation*}
    という制約を満たす必要がある．また，この表記\eqref{eq:integer-radix-N-expression}自体の意味は，
    \begin{equation}
        x = x_0 N^0 + x_1 N^1 + \cdots + x_n N^n  \label{eq:integer-radix-N-interpretation}
    \end{equation}
    という式が主張する通りである．
    ...
    \begin{dfn}{正整数の$N$進法表記}{radix-N-integer}
        ...
        整数の有限列$x_0, x_1, ..., x_n$（ただし$n$は$0$以上の整数）
        が以下の条件1,2,3を満たすとき，この有限列\footnote{
            通常は式\eqref{eq:integer-radix-N-expression}のことをこう呼ぶが，
            数学的にはこう定義した方が納まりがよい．
        }を$x$の$N$\ind{進法表記}{しんほうひょうき}といい，$x$を
        \begin{equation*}
        ...
        \begin{enumerate}
            ...
            \item $x = x_0 N^0 + x_1 N^1 + \cdots + x_n N^n$
            \item $x_n \neq 0$
        \end{enumerate}
    \end{dfn}
